 ...
 */
public class ViewPayment extends android.support.v4.app.Fragment {
    RefreshLayout refreshLayout;
    ListView listView;
    View footerLayout;
    JSONArray temp;
    CustomAdapterPayments customAdapterPayments;
    Boolean empty=true;
    ...
    public View onCreateView(LayoutInflater inflater, final ViewGroup container,
       ...
       final View view  = inflater.inflate(R.layout.fragment_view_payment, container, false);
        ...
        listView.setAdapter(customAdapterPayments);
        ...
        asyncHttpClient.post("http://54.149.252.188/TipM/GetAllPayments.php", requestParams, new JsonHttpResponseHandler() {
            @Override
            public void onSuccess(int statusCode, Header[] headers, JSONArray response) {
                if (response.isNull(0)) {
                    empty=true;
                    String[] payments = {"No payments made yet"};
                    listView.setAdapter((new ArrayAdapter<String>(getActivity(), R.layout.empty_layout,R.id.empty_textview, payments)));

                } else {
                    empty = false;
                    customAdapterPayments = new CustomAdapterPayments(getActivity(), response);
                    listView.setAdapter(customAdapterPayments);
                }
            }
        });
        ...
        refreshLayout.setOnLoadListener(new RefreshLayout.OnLoadListener() {
            ...
            public void onLoad() {
                ...
                asyncHttpClient.post(getActivity(), "http://54.149.252.188/TipM/GetAllPayments.php", request, new JsonHttpResponseHandler() {
                    @Override
                    public void onSuccess(int statusCode, Header[] headers, JSONArray response) {
                        if (response.isNull(0)) {
                            refreshLayout.setLoading(false);
                        } else {
                            customAdapterPayments.add(response);
                            customAdapterPayments.notifyDataSetChanged();
                            refreshLayout.setLoading(false);
                        }
        ...
        refreshLayout.setOnRefreshListener(new SwipeRefreshLayout.OnRefreshListener() {
            ...
            public void onRefresh() {
                ...
                if (empty == true) {
                    ...
                    firstObject = customAdapterPayments.getItem(0);
                }
                ...
                asyncHttpClient1.post(getActivity(), "http://54.149.252.188/TipM/RefreshPayments.php", request, new JsonHttpResponseHandler() {
                    @Override
                    public void onSuccess(int statusCode, Header[] headers, JSONArray response) {
                        if (response.isNull(0)) {
                            ...
                            if (empty == true) {
                                ...
                                customAdapterPayments.addRefresh(response);
                                customAdapterPayments.notifyDataSetChanged();
                                refreshLayout.setRefreshing(false);
                            }
                        }
